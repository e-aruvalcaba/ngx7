...
```
## Cypress
Added two new input properties so you can add `data-tid` to the native HTML input and label tags:
- `dataTIdRadio` for the data-tid attribute on the input tag
- `dataTIdLabel` for the data-tid attribute on the label tag

So this code:
`<cmx-radio-button [dataTIdRadio]="'rdbtn-gender-male'" [dataTIdLabel]="'lbl-gender-male'"> Male</cmx-radio-button>` will render this HTML:
```
<input type="radio" data-tid="rdbtn-gender-male" />
<label data-tid="lbl-gender-male">Male</label>
```
